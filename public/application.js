...
// UPDATE ONE
var updateOneItem = function updateOneItem(data) {
...
// DELETE ONE
var deleteOneItem = function deleteOneItem(id) {
  return $.ajax({
    method: 'DELETE',
...
var showItemsTemplate = __webpack_require__(16);
var showItemsTemplate2 = __webpack_require__(33);
var store = __webpack_require__(2);
var itemsApi = __webpack_require__(4);

// Clear modal forms
var clearModalForms = function clearModalForms() {
  $('#updateInputName').val('');
  $('#updateInputDate').val('');
...
// Clear item-bucket
var clearItemBucket = function clearItemBucket() {
  $('#item-bucket').html('');
};

// Chech active status to add active-color and active-label classes
var checkActiveStatus = function checkActiveStatus(data) {
...
var createAllItemBoxes = function createAllItemBoxes(data) {
  // console.log('createAllItemBoxes data is ', data)
  var showItemsHtml = showItemsTemplate2({ items: data.items });
  $('#item-bucket').append(showItemsHtml);
  var itemsCreated = document.getElementsByClassName('bucket-2');
  // console.log('itemsCreated is ', itemsCreated)
  // converts htmlCollection into array
  var arr = Array.prototype.slice.call(itemsCreated);
...
var createOneItemBox = function createOneItemBox(data) {
  // console.log('createOneItemBox data is ', data)
  var showItemsHtml = showItemsTemplate({ items: data });
  // console.log('showItemsHtml is ', showItemsHtml)
  $('#item-bucket').append(showItemsHtml);
  var checkNewItem = $('#item-bucket .bucket').last();
...
// Create item success
var createItemSuccess = function createItemSuccess(createResponse) {
...
// Create item error
var createItemError = function createItemError(createError) {
  // console.log('createError is ', createError)
  $('.create-item-alert-container').html('<div class="alert alert-danger alert-change-pw-error">' + '<button type="button" class="close" aria-hidden="true" data-dismiss="alert">&times;</button>' + 'Create item was unsuccessful </div>');
...
var removeOneItemBox = function removeOneItemBox(itemID) {
...
var showAllSuccess = function showAllSuccess(showAllResponse) {
  console.log('showAllSuccess ran');
  if (showAllResponse.items.length > 0 && $('#item-bucket').html() !== '') {
    $('.full-width-alert-container').html('<div class="alert alert-success alert-user-items-refreshed">' + '<button type="button" class="close" aria-hidden="true" data-dismiss="alert">&times;</button>' + 'Items view refreshed. </div>');
  ...
  if (showAllResponse.items.length === 0) {
    $('.full-width-alert-container').html('<div class="alert alert-success alert-user-items-empty">' + '<button type="button" class="close" aria-hidden="true" data-dismiss="alert">&times;</button>' + 'No items to show. Create a new item to start. </div>');
...
var showAllError = function showAllError(_showAllError) {}
// console.log('showAllError is ', showAllError)


// Populate modal with one item
;var populateItemInModal = function populateItemInModal(itemData) {
...
var updateOneSuccess = function updateOneSuccess(updateResponse) {
  // console.log('updateResponse is ', updateResponse)
  $('#updateModal').modal('toggle');
  itemsApi.showAllItems().then(showAllSuccess).catch(showAllError);
...
var updateOneError = function updateOneError(_updateOneError) {
  // console.log('updateOneError is ', updateOneError)
  $('.form-horizontal[data-id=' + store.update.item.id + ']').append('<div class="alert alert-danger alert-update-error">' + '<button type="button" class="close" aria-hidden="true" data-dismiss="alert">&times;</button>' + 'Update item was unsuccessful. </div>');
  $('.alert-update-error').delay(3000).fadeOut();
};

// create delete confirmation alert
var createDeleteConfAlert = function createDeleteConfAlert(itemData) {
  console.log('createDeleteConfAlert itemData is', itemData);
  $('.bucket-2[data-id=' + itemData.id + ']').append('<div class="alert alert-warning fade in item-alert" role="alert">' + '<div><a href="#" class="alert-link">Are you sure you want to delete ' + itemData.name + '?</a></div>' + '<br>' + '<button type="button" class="btn btn-default delete-item-conf-btn">Delete</button>' + '<button type="button" class="btn btn-default" data-dismiss="alert" aria-label="Close">Cancel</button>' + '</div>');
...
// Delete one success
var deleteOneSuccess = function deleteOneSuccess() {
...
// Delete one error
var deleteOneError = function deleteOneError() {
  // console.log('deleteOneError ran')
  $('.full-width-alert-container').html('<div class="alert alert-danger alert-delete-item-error">' + '<button type="button" class="close" aria-hidden="true" data-dismiss="alert">&times;</button>' + 'Delete item was unsuccessful. </div>');
  $('.alert-delete-item-error').delay(3000).fadeOut();
};

module.exports = {
  clearModalForms: clearModalForms,
  clearItemBucket: clearItemBucket,
  createItemSuccess: createItemSuccess,
  createItemError: createItemError,
  createAllItemBoxes: createAllItemBoxes,
  createOneItemBox: createOneItemBox,
  removeOneItemBox: removeOneItemBox,
  showAllSuccess: showAllSuccess,
  showAllError: showAllError,
  populateItemInModal: populateItemInModal,
  updateOneSuccess: updateOneSuccess,
  updateOneError: updateOneError,
  createDeleteConfAlert: createDeleteConfAlert,
  deleteOneSuccess: deleteOneSuccess,
...
var apiUrl = void 0;
var apiUrls = {
  production: 'https://aqueous-forest-96537.herokuapp.com',
...
var addNestedValue = __webpack_require__(34);
...
var itemsEvents = __webpack_require__(35);
...
var authUi = __webpack_require__(15);
var authApi = __webpack_require__(9);
var store = __webpack_require__(2);
...
var onSignUp = function onSignUp(event) {
  ...
  if (data.credentials.password === data.credentials.password_confirmation) {} else {
...
var onChangePW = function onChangePW(event) {
  event.preventDefault();
  var data = getFormFields(event.target);
  if (data.passwords.old !== data.passwords.new) {} else {
    authUi.pwMatching();
    return;
  }
  // api
  authApi.changePW(data).then(authUi.changePWSuccess).catch(authUi.changePWError);
};
...
var store = __webpack_require__(2);
var itemsUi = __webpack_require__(5);
var itemsApi = __webpack_require__(4);
var authApi = __webpack_require__(9);
...
var pwNotMatching = function pwNotMatching() {
  clearAlerts();
  $('.sign-up-alert-container').html('<div class="alert alert-danger alert-sign-up-error">' + 'Passwords do not match </div>');
...
// PASSWORD do not match
var pwMatching = function pwMatching() {
  clearAlerts();
  $('.change-pw-alert-container').html('<div class="alert alert-danger alert-change-pw-error">' + 'New password must be different than old </div>');
  $('.alert-change-pw-error').delay(3000).fadeOut();
...
var signUpError = function signUpError() {
  ...
  clearAlerts();
  $('.sign-up-alert-container').html('<div class="alert alert-danger alert-sign-up-error">' + 'Sign up was unsuccessful </div>');
  $('.alert-sign-up-error').delay(3000).fadeOut();
  clearForms();
};
...
var signUpSuccess = function signUpSuccess(signUpResponse) {
  ...
  var onSignInAfterUp = function onSignInAfterUp() {
    // console.log('signing in after a sign up')
    // console.log('store.credentials is', store.credentials)
    authApi.signIn(store).then(signInSuccess).catch(signInError);
  };
  onSignInAfterUp();
  clearForms();
};
...
var signInSuccess = function signInSuccess(signInResponse) {
  ...
  clearForms();
  itemsApi.showAllItems().then(itemsUi.showAllSuccess).catch(itemsUi.showAllError);
};

// SIGN IN error
var signInError = function signInError() {
  clearAlerts();
  $('.sign-in-alert-container').html('<div class="alert alert-danger alert-sign-in-error">' + 'Sign in was unsuccessful </div>');
...
var changePWSuccess = function changePWSuccess() {
  ...
  $('.full-width-alert-container').html('<div class="alert alert-success alert-change-pw-success">' + '<button type="button" class="close" aria-hidden="true" data-dismiss="alert">&times;</button>' + 'Password was successfully updated for: ' + store.user.email + '</div>');
  $('.alert-change-pw-success').delay(3000).fadeOut();
  $('#change-pw-btn').dropdown('toggle');
  clearForms();
};
// Change PW error
var changePWError = function changePWError() {
  clearAlerts();
  $('.change-pw-alert-container').html('<div class="alert alert-danger alert-change-pw-error">' + 'Password change was unsuccessful </div>');
  $('.alert-change-pw-error').delay(3000).fadeOut();
...
var signOutSuccess = function signOutSuccess() {
  ...
  $('.full-width-alert-container').html('<div class="alert alert-success alert-sign-out-success full-width-alert">' + '<button type="button" class="close" aria-hidden="true" data-dismiss="alert">&times;</button>' + 'Signed out successfully. </div>');
...
// SIGN OUT error
var signOutError = function signOutError() {
  clearAlerts();
  $('.full-width-alert-container').html('<div class="alert alert-danger alert-sign-out-error">' + '<button type="button" class="close" aria-hidden="true" data-dismiss="alert">&times;</button>' + 'Failed to sign out. </div>');
...
module.exports = {
  ...
  pwNotMatching: pwNotMatching,
  pwMatching: pwMatching,
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
  ...
  return "\n\n<form class=\"form-horizontal bucket\" data-id=\""
    ...
    + "</p>\n    </div>\n    <label class=\"control-label col-sm-3\" for=\"active\">Active:</label>\n    <div class=\"col-sm-9\">\n      <p class=\"form-control-static item-active-status\">"
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.active : stack1), depth0))
    + "</p>\n    </div>\n    <div class=\"btn-group\">\n      <button type=\"button\" class=\"btn btn-default edit-item-btn\" aria-label=\"edit item\">\n        <span class=\"glyphicon glyphicon-pencil\" aria-hidden=\"true\" ></span>\n      </button>\n      <button type=\"button\" class=\"btn btn-default delete-item-btn\" aria-label=\"delete item\">\n        <span class=\"glyphicon glyphicon-trash\" aria-hidden=\"true\" ></span>\n      </button>\n    </div>\n  </div>\n</form>\n\n";
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
  ...
  return "\n<div class=\"bucket-2\" data-id=\""
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
    + "\">\n  <div class=\"row\">\n    <div class=\"col-sm-10 item-card item-name\">"
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.name : stack1), depth0))
    + "</div>\n    <div class=\"col-sm-2 item-card item-label item-active-status glyphicon\" data-status=\""
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.active : stack1), depth0))
    + "\"></div>\n  </div>\n  <hr/>\n  <div class=\"row\">\n    <div class=\"col-sm-1 item-card item-label glyphicon glyphicon-calendar\"></div>\n    <div class=\"col-sm-11 item-card\">"
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.date : stack1), depth0))
    + "</div>\n  </div>\n  <hr/>\n  <div class=\"row\">\n    <div class=\"col-sm-1 item-card item-label glyphicon glyphicon-map-marker\"></div>\n    <div class=\"col-sm-11 item-card\">"
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.address : stack1), depth0))
    + "</div>\n  </div>\n  <hr/>\n  <div class=\"row\">\n    <div class=\"col-sm-1 item-card item-label glyphicon glyphicon-link\"></div>\n    <div class=\"col-sm-11 item-card\"><a href=\""
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.url : stack1), depth0))
    + "\">"
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.url : stack1), depth0))
    + "</a></div>\n  </div>\n  <hr/>\n  <div class=\"row\">\n    <div class=\"col-sm-1 item-card item-label glyphicon glyphicon-time\"></div>\n    <div class=\"col-sm-11 item-card\">"
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.hours : stack1), depth0))
    + "</div>\n  </div>\n  <hr/>\n  <div class=\"row\">\n    <div class=\"col-sm-1 item-card item-label glyphicon glyphicon-hourglass\"></div>\n    <div class=\"col-sm-5 item-card item-border-right\">"
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.duration : stack1), depth0))
    + "</div>\n    <div class=\"col-sm-1 item-card item-label glyphicon glyphicon-jpy\"></div>\n    <div class=\"col-sm-5 item-card\">"
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.cost : stack1), depth0))
    + "</div>\n  </div>\n  <hr/>\n  <div class=\"row\">\n    <div class=\"col-sm-1 item-card item-label glyphicon glyphicon-tag\"></div>\n    <div class=\"col-sm-11 item-card\">"
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.tags : stack1), depth0))
    + "</div>\n  </div>\n  <hr/>\n  <div class=\"row\">\n    <div class=\"col-sm-1 item-card item-label glyphicon glyphicon-info-sign\"></div>\n    <div class=\"col-sm-11 item-card\">"
...
var onCreateItem = function onCreateItem(event) {
  ...
  // debugger
  // api
  itemsApi.createItem(data).then(itemsUi.createItemSuccess).catch(itemsUi.createItemError).then(onShowAllItems);
};

// SHOW ALL ITEMS from server
var onShowAllItems = function onShowAllItems(event) {
  // console.log('show all items was clicked')
  itemsApi.showAllItems().then(itemsUi.showAllSuccess).catch(itemsUi.showAllError);
};

// HIDE ALL ITEMS
var onHideAllItems = function onHideAllItems() {
  // console.log('onHideAllItems ran')
  // check if user has any items
  if ($.trim($('#item-bucket').html()) === '') {
    $('.full-width-alert-container').html('<div class="alert alert-success alert-user-items-empty">' + '<button type="button" class="close" aria-hidden="true" data-dismiss="alert">&times;</button>' + 'No items to hide. </div>');
    $('.alert-user-items-empty').delay(3000).fadeOut();
    // console.log('items is empty')
    // debugger
    return;
...
var onOpenCreateItem = function onOpenCreateItem(event) {
  $('.create-dropdown').dropdown('toggle');
  // console.log('onOpenCreateItem ran')
};

// OPEN UPDATE MODAL
var onOpenUpdate = function onOpenUpdate(event) {
  console.log('open update was clicked');
  var dataID = event.currentTarget.parentElement.parentElement.getAttribute('data-id');
  console.log('dataID is ', dataID);
  dataID = Number(dataID);
  var itemData = store.items.find(function (x) {
    return x.id === dataID;
  });
...
var onOpenDelete = function onOpenDelete(event) {
...
var onConfirmDeleteItem = function onConfirmDeleteItem(event) {
  ...
  console.log('store.delete is ', store.delete);
  itemsApi.deleteOneItem(store.delete).then(itemsUi.deleteOneSuccess).catch(itemsUi.deleteOneError);
  // debugger
};
...
var onUpdateItem = function onUpdateItem(event) {
  ...
  // debugger
  itemsApi.updateOneItem(store.update).then(itemsUi.updateOneSuccess).catch(itemsUi.updateOneError);
};

module.exports = {
  onCreateItem: onCreateItem,
  onShowAllItems: onShowAllItems,
  onHideAllItems: onHideAllItems,
